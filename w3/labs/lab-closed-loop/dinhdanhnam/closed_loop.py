...
def resolve_script(script: str, cfg: dict | None = None) -> str:
    path = Path(script)
    if path.is_absolute():
        return str(path)
    if cfg and cfg.get("_config_dir"):
        return str((Path(cfg["_config_dir"]) / path).resolve())
    return str(path)
...
def run_transactional_steps(
    ...
    completed: list[str] = []
    for step in steps:
        if not run_runbook(step, service, dry_run=dry_run, timeout_s=timeout_s, cfg=cfg):
...
def _process_alert_locked(
    ...
    if not run_runbook(runbook, service, dry_run=True, timeout_s=timeout_s, cfg=cfg):
    ...
    if multi_steps:
        ...
        success, completed = run_transactional_steps(multi_steps, service, False, timeout_s, cfg)
        if not success:
            ...
            for rb_step in reversed(rollback_steps[: len(completed)]):
                ...
                run_runbook(rb_step, service, dry_run=False, timeout_s=timeout_s, cfg=cfg)
            log.info("TRANSACTIONAL_ROLLBACK_COMPLETE", service=service,
                     rolled_back=list(reversed(rollback_steps[: len(completed)])))
            cb.record_failure()
            return
    else:
        if not run_runbook(runbook, service, dry_run=False, timeout_s=timeout_s, cfg=cfg):
    ...
    run_runbook(rollback, service, dry_run=False, timeout_s=timeout_s, cfg=cfg)
